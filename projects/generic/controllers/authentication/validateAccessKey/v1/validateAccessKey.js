...
var Shared = require('mia-js-core').Shared
var RateLimiter = require('mia-js-core').RateLimiter
...
function ThisModule () {
  ...
  var _checkValidTokenDate = function (options, maxTokenValidTime, tokenTimestamp) {
    ...
    if (tokenDate == 'Invalid Date') { // eslint-disable-line eqeqeq
  ...
  var _generateAuthorizedSignature = function (req, deviceId, secret, timeStamp, signatureMethod) {
    ...
    var baseUrl = protocol.toLowerCase() + '://' + req.headers.host.toLowerCase()
    //, decoder = new StringDecoder('utf8')
    var bodyHash = req.rawBodyJSON ? Crypto.createHash(signatureMethod).update(req.rawBodyJSON.toLowerCase(), 'utf8').digest('hex') : ''
    var urlPath = _.get(req, 'miajs.originalUrl') || req.url

    // decoder.write(new Buffer(decodeURI(req.url)))
    var hashString = (deviceId + secret + timeStamp + requestMethod + baseUrl + urlPath + bodyHash).toLowerCase() // Append hash components
  ...
  var _validateSignature = function (req) {
    ...
    var options = { translator: translator }
    var signatureToken = req.miajs.accessKeyParameters.key
    var signatureMethod = req.miajs.accessKeyParameters.signaturemethod || 'sha256'
    var deviceId = signatureToken.substring(0, 32)
    var secretId = signatureToken.substring(32, 64)
    var signatureTimeStamp = signatureToken.substring(64, 74)
    var signature = signatureToken.substring(74, signatureToken.length)
    var group = req.miajs.route.group
    var maxTokenValidTime = 300 // Max time token is valid in seconds
    ...
    if (signatureDate == 'Invalid Date') { // eslint-disable-line eqeqeq
  ...
  var _validateAccessKey = function (req, res, next) {
    ...
    var options = { translator: translator }
    var protocol = req.header('X-Forwarded-Proto') || req.protocol
    var key = req.miajs.accessKeyParameters.key
