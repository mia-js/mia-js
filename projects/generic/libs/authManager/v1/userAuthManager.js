...
var ObjectID = require('mia-js-core/lib/dbAdapters').MongoObjectID
...
var UserProfileModel = Shared.models('generic-userProfile-model')
...
function ThisModule () {
  ...
  self.processProfileData = function (params) {
    ...
    return Q().then(function () {
      ...
      if (_.isObject(params.userProfileModel) && params.userProfileModel != UserProfileModel) { // eslint-disable-line eqeqeq
        // Use custom model
      ...
      } else if (params.userProfileModel == 'doNotValidate') { // eslint-disable-line eqeqeq
  ...
  var _beforeProfileUpdate = function (params) {
    ...
    if (!_.isEmpty(params.login) && params.login != params.userData.login) { // eslint-disable-line eqeqeq
      return UserModel.findOne({
        login: params.login,
        group: params.group
  ...
  var _updateUserProfileDataCore = function (params) {
    ...
    return Q().then(function () {
      ...
      if (!_.isEmpty(params.login) && params.login != userData.login) { // eslint-disable-line eqeqeq
